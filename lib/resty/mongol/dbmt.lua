...
function dbmethods:auth_mongo_cr(username, password)
...
-- https://github.com/mongodb/mongo/blob/dd15f9c211f5bc40b6fa8a7cd44350d3aeb87d89/src/mongo/db/wire_version.h
function dbmethods:auth(username, password)
    local db = self.db
    self.db = "admin"
    local r, err = self:cmd({ismaster = true})
    if not r then
        return nil, err
    else
        self.db = db
    ...
    if r.maxWireVersion < 3 then
        return self:auth_mongo_cr(username, password)
    else
        return self:auth_scram_sha1(username, password)
    end
end
